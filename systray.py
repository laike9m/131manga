...
class SysTrayIcon(object):
    ...
    def __init__(self,               
        ...
        menu_options = menu_options + (('退出程序', None, self.QUIT),)#改了选项的文字
        ...
        self.window_class = win32gui.WNDCLASS() #把局部变量window_class改成类的属性
        self.hinst = self.window_class.hInstance = win32gui.GetModuleHandle(None)
        ...
        self.classAtom = win32gui.RegisterClass(self.window_class)  #classAtom也改成属性,后面unregister时传入
        # Create the Window.
    ...
    def _add_ids_to_menu_options(self, menu_options):
        ...
        for menu_option in menu_options:
            option_text, option_icon, option_action = menu_option   #option各项的含义
    ...
    def destroy(self, hwnd, msg, wparam, lparam):
        #if self.on_quit: self.on_quit(self)    注释掉,不然每次托盘消失时都会调用on_quit函数
        nid = (self.hwnd, 0)
        win32gui.Shell_NotifyIcon(win32gui.NIM_DELETE, nid)
        win32gui.PostQuitMessage(0) # Terminate the app.

    def icon_exit_show_window(self):
        '''这个我添加的函数用于完成注销托盘图标并让原窗口显示'''
        self.gui.tk.deiconify()
        win32gui.DestroyWindow(self.hwnd)   #毁掉托盘
        win32gui.UnregisterClass(self.classAtom,self.hinst)#注销
        
    def notify(self, hwnd, msg, wparam, lparam):
        if lparam==win32con.WM_LBUTTONDBLCLK:
            #self.execute_menu_option(self.default_menu_index + self.FIRST_ID)  注释掉它
            self.icon_exit_show_window()    #我添加
...
def non_string_iterable(obj):
    try:
        ...
        return not (isinstance(obj,str) or isinstance(obj,bytes) or isinstance(obj,bytearray))
...
if __name__ == '__main__':
    ...
    hover_text = "SysTrayIcon.py Demo"  #鼠标移动到tray上,显示的文字
    ...
    menu_options = (('Say Hello', icons.__next__(), hello),
                    ...
                    ('A sub-menu', icons.__next__(), (
                                                      ('Say Hello to Simon', icons.__next__(), simon),
